...
Hydrological datasets, sometimes collected over decades, may contain missing recordings. Depending on the recording issue, this could be an hour, a day, or a year. Let us define a simple timeseries with data from 4 different weather stations over 5 different days. Some of the readings are missing and their entry contains a `NA`.
...
Set your working directory accodingly using `setwd()`. Be sure that the file `allprcp.Rdata` is in this dir.
```{r}
load("allprcp.Rdata")
```
...
**Question 2.3** What is the column range of the data? (Begins with US....) Use this information to make a new data frame called rain.
```{r, eval=FALSE}
...
1. Instead of writing the output of `addNA()` every iteration, `foreach` arranges the ouput and gives us the output at the end. See `NAs_in_rain_par = foreach` and the `.combine` option.  
...
**Question 3.3** What is the elapsed time of this code? Is it faster than our earlier serial code?
